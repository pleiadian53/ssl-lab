...
The method has three trainable components, and each has its own runnable script in the example folder. The letters A, B, and C follow the *conceptual* order from [Chapter 1](01-the-approach.md): the encoder first, then the decoder that closes the latent-to-data gap, then the flow that closes the point-to-distribution gap. The script numbers follow their own file-ordering convention, so the letters and the numbers do not line up one to one. Read the letters as the idea and the numbers as the files.
...
The `combo` split is the generalization test, and it is the Norman headline. It holds out $20$ two-gene combinations under a specific and demanding condition: for each held-out combination, *both* of its constituent single genes are seen individually during training. The model therefore has met each part but never their conjunction, which is exactly the compositional generalization a gene-set embedding is meant to deliver. The training side of `combo` spans $197$ perturbations covering $105$ distinct target genes.

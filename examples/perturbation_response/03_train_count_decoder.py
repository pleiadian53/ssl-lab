...
def main() -> None:
    ...
    decoder = CountDecoder(latent_dim=jepa.cfg.embed_dim, n_genes=n_genes, zinb=args.zinb).to(device)
    opt = torch.optim.AdamW(decoder.parameters(), lr=args.lr)
    logger.info("encoder dim=%d  genes=%d  zinb=%s  train cells=%d",
                jepa.cfg.embed_dim, n_genes, args.zinb, len(train_loader.dataset))
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            loss = decoder.nll(z, counts, libsize)
            opt.zero_grad()
            loss.backward()
            opt.step()
            running += loss.item()
    ...
    torch.save({"state": decoder.state_dict(), "latent_dim": jepa.cfg.embed_dim,
                "n_genes": n_genes, "zinb": args.zinb}, out)

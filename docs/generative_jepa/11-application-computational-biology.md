...
> **Recap — where this sits.** [Parts 6–10](05-two-gaps-four-routes.md) built five ways to turn a JEPA encoder into a generator: decode the latent (A), a variational posterior (B), conditioned diffusion (C), the conditional flow prior that unifies them, and planning on top (D). This chapter does not introduce new machinery — it **assembles** those pieces into a concrete model for one of the two domains that motivated the whole series: predicting single-cell responses to perturbations, the central problem in computational drug discovery. The perturbation setup is reintroduced here from scratch; for the data itself the [data-modalities primer](appendix-data-modalities.md) covers it, and for a deeper standalone treatment (datasets, baselines, the broader modeling progression) see the optional companion [genai-lab perturbation-prediction guide](https://github.com/pleiadian53/genai-lab/blob/main/docs/applications/perturbation_prediction.md).

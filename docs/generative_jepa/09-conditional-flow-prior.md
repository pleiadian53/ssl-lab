...
*The Parts 0–4 starter could generate a plausible latent, but never a latent given a condition. This chapter adds the one missing piece: a single condition input to the flow's velocity field. That turns the starter into a conditional generator, and it turns out to be the same model the variational route (Part 7) and the diffusion route (Part 8) were each circling.*
...
If diffusion (Route C) and flow are siblings, why give flow its own chapter rather than fold it into Route C? Because the *flow* realization has a distinct practical profile worth stating, and because it is the one this series actually builds.
...
And the honest notes, unchanged in spirit from the rest of the series: structurally this is *latent flow-matching generation with JEPA as encoder pretraining*. That pairing *could* be a strong stack — diffusion, flow's close sibling, has already been used to extend JEPA in much this spirit — and whether it is one here is exactly what the implementation under [`examples/perturbation_response`](../../examples/perturbation_response/) is built to find out. The generative power is the flow's; JEPA only contributes the representation the flow is conditioned on, and that ongoing effort is where we **show** whether conditioning on a JEPA latent earns its keep over a cheaper representation. On the open likelihood question ([Part 5 §5](05-two-gaps-four-routes.md)), the flow is in the same place as Route C: an in-principle density via its ODE, broken in *data* space by any decoder that sits after the flow.
...
- **Condition injection.** Feed $c$ into the encoder itself (via FiLM or cross-attention) so its output depends on the intervention, giving a "state-*under-condition*" encoder rather than a pure "state" encoder. Whether this *scales* turns not on the injection but on the **condition's representation**. Keyed on a finite per-condition lookup, it hits the same wall as a per-perturbation table: an intervention never seen has no vector to inject, the limitation the compositional gene-set encoder dissolves downstream. Only a **parametric** condition, assembled from reusable parts or a continuous parameterization, lets injection reach rich or unbounded interventions. Injection also forfeits an efficiency the decoupled design keeps. Once the encoder's output depends on $c$, the representation is no longer computed once and reused across conditions; it must be recomputed per condition.
...
Two axes are hiding in that dial, and separating them sharpens the choice. One is *where* conditioning enters: downstream, injected, or native (the three settings above). The other is *how rich the condition itself can be*: a fixed menu of interventions, or a **parametric** one that composes and interpolates. The second axis governs reach, and it is exactly what the action-operator view supplies. Read the condition as a parametric operator $f_{\theta(c)}$ and conditioning becomes something *generated from structure* rather than *enumerated*, compositional and unbounded whichever end of the where-axis you sit at. That is why the deepest setting is not just "condition earlier" but "condition through an operator."

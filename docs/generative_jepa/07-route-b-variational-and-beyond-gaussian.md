...
> **Prior vs. posterior, disambiguated.** If that distinction feels slippery — especially without a variational-autoencoder background — the companion [Part 7c](07c-which-network-is-the-prior.md) builds it from scratch: what counts as "the data" here, why the conditioning context is *not* the prior, the two architectures (amortized vs. recognition) that both make Route B, a wiring diagram for the recognition form, and how the action operator generalizes the whole picture.
